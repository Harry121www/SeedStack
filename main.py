...
def extract_files(text: str) -> list:
    ...
    if m and len(m) >= 2:
        return [(x[0].strip(), x[1].strip()) for x in m]
    ...
    # 策略3: ### N. path/file.ext + ```lang\ncode\n```
    p = r'###\s*\d+\.\s*(.+?\.\w+)\s*\n\s*```\w*\s*\n(.*?)```'
    m = re.findall(p, text, re.DOTALL)
    if m and len(m) >= 2:
        return [(x[0].strip(), x[1].strip()) for x in m]

    # 策略4: **path/file.ext** + ```lang\ncode\n```
    p = r'\*\*(.+?\.\w+)\*\*\s*\n\s*```\w*\s*\n(.*?)```'
    m = re.findall(p, text, re.DOTALL)
    if m and len(m) >= 2:
        return [(x[0].strip(), x[1].strip()) for x in m]
    ...
    # 策略6: 路径代码块 ```path/to/file.ext\ncode\n```
    p = r'```([\w./-]+/[\w./-]+\.\w+)\s*\n(.*?)```'
    m = re.findall(p, text, re.DOTALL)
    if m and len(m) >= 2:
        return [(x[0].strip(), x[1].strip()) for x in m]

    # 策略7: 宽松匹配 - 任何 "N. path" 后跟代码块(跨段落)
    # e.g. "1. src/main/java/com/huini/service/AuthService.java\n```java\ncode\n```"
    p = r'(?:^|\n)\s*\d+\.\s*([\w./-]+\.\w+)\s*\n\s*```\w*\s*\n(.*?)```'
    m = re.findall(p, text, re.DOTALL)
    if m and len(m) >= 2:
        return [(x[0].strip(), x[1].strip()) for x in m]

    # 策略8: "## N. path" Markdown标题格式 + 代码块
    # e.g. "## 1. frontend/package.json\n\n```json\ncode\n```"
    p = r'##\s*\d+\.\s*([\w./-]+\.\w+)\s*\n\s*```\w*\s*\n(.*?)```'
    m = re.findall(p, text, re.DOTALL)
    if m and len(m) >= 2:
        return [(x[0].strip(), x[1].strip()) for x in m]

    # 策略9: JSON格式 {"files": [{"path": "...", "content": "..."}]}
    ...
    # 策略10: "### N. `path`" 反引号包裹路径 + 代码块
    # e.g. "### 1. `AuthController.java`\n\n```java\ncode\n```"
    p = r'###\s*\d+\.\s*`([^`]+)`\s*\n\s*```\w*\s*\n(.*?)```'
    m = re.findall(p, text, re.DOTALL)
    if m and len(m) >= 2:
        return [(x[0].strip(), x[1].strip()) for x in m]

    # 策略11: 代码块内以 "## N. path" 开头的分割格式
    # First, find all "## N. path" markers with their positions
    ...
    if len(found) >= 2:
        ...
        for j, (line_idx, filepath) in enumerate(found):
            # Code starts after this marker line
            start = line_idx + 1
            # Code ends at the next marker or end of text
...
def fill(t, **kw):
    for k, v in kw.items():
        t = t.replace("__" + k + "__", v)
    return t


def _collect_source_files(base_dir: str, exts: tuple) -> str:
    """Collect source files content for error fixing prompt."""
    result = ""
...
def _fix_and_rewrite(error_output: str, base_dir: str, label: str):
    """Feed errors to LLM, get fixed files, rewrite them."""
...
def generate_docs(state: WorkflowState) -> dict:
    tpl = """你是一个资深的产品经理和技术架构师。根据以下需求生成两份文档。
...
"""
    resp = model.invoke(fill(tpl, REQUIREMENT=state["requirement"]))
    c = resp.content
    rd, ad = "", ""
    if "---需求文档---" in c and "---API接口文档---" in c:
        p = c.split("---需求文档---")[1]
        if "---API接口文档---" in p:
            rd = p.split("---API接口文档---")[0].strip()
            ad = p.split("---API接口文档---")[1].strip()
    else:
        rd = ad = c
    print("[1/8] 文档完成 (需求%d字, API%d字)" % (len(rd), len(ad)))
    return {"requirement_doc": rd, "api_doc": ad, "code_parts": []}


# ====== 2. 后端基础设施 ======
def gen_backend_infra(state: WorkflowState) -> dict:
    tpl = """你是Spring Boot专家。生成后端基础设施代码(Java 17, Spring Boot 3.2, JPA, MySQL, JWT, Lombok)。
...
"""
    ...
    resp = model.invoke(fill(tpl, REQ_DOC=state["requirement_doc"][:2000]))
    fs = extract_files(resp.content)
    print("  %d个: %s" % (len(fs), [f[0] for f in fs]))
    return {"code_parts": state["code_parts"] + [resp.content]}


# ====== 3. 实体+枚举+仓库+DTO ======
def gen_backend_domain(state: WorkflowState) -> dict:
    tpl = """你是Spring Boot专家。生成Entity/Enum/Repository/DTO(Java17, Lombok, JPA, 包com.huini)。
...
"""
    ...
    resp = model.invoke(fill(tpl, API_DOC=state["api_doc"][:2000]))
    fs = extract_files(resp.content)
    print("  %d个: %s" % (len(fs), [f[0] for f in fs]))
    return {"code_parts": state["code_parts"] + [resp.content]}
...
# ====== 4. Service层 ======
def gen_services(state: WorkflowState) -> dict:
    tpl = """你是Spring Boot专家。生成Service层(包com.huini.service, @Service, @RequiredArgsConstructor, @Transactional)。
...
"""
    ...
    fs = extract_files(resp.content)
    if len(fs) == 0:
        debug_path = os.path.join(os.path.dirname(__file__), "debug_step4_response.txt")
        with open(debug_path, "w", encoding="utf-8") as f:
            f.write(resp.content[:3000])
...
# ====== 5. Controller + 数据初始化 ======
def gen_controllers(state: WorkflowState) -> dict:
    tpl = """你是Spring Boot专家。生成Controller和数据初始化代码。
...
"""
    ...
    fs = extract_files(resp.content)
    if len(fs) == 0:
        debug_path = os.path.join(os.path.dirname(__file__), "debug_step5_response.txt")
        with open(debug_path, "w", encoding="utf-8") as f:
            f.write(resp.content[:3000])
        print(f"  [调试] 0个文件, 保存到 {debug_path}")
    print("  %d个: %s" % (len(fs), [f[0] for f in fs]))
...
# ====== 6a. Vue 3 前端 - 配置+组件 ======
def gen_vue_config_components(state: WorkflowState) -> dict:
    tpl = """你是Vue 3 + Vite前端架构师。为汇尼易链(易货交易平台)生成前端配置文件和公共组件。
...
"""
    ...
    resp = model.invoke(fill(tpl, REQ_DOC=state["requirement_doc"][:1000]))
    fs = extract_files(resp.content)
    if len(fs) == 0:
        debug_path = os.path.join(os.path.dirname(__file__), "debug_vue_part1.txt")
        with open(debug_path, "w", encoding="utf-8") as f:
            f.write(resp.content[:3000])
        print(f"  [调试] 0个文件, 保存到 {debug_path}")
    print("  %d个: %s" % (len(fs), [f[0] for f in fs]))
...
# ====== 6b. Vue 3 前端 - 视图页面 ======
def gen_vue_views(state: WorkflowState) -> dict:
    tpl = """你是Vue 3 + Vite前端架构师。为汇尼易链生成所有视图页面。
...
"""
    ...
    resp = model.invoke(fill(tpl, REQUIREMENT=state["requirement_doc"][:500]))
    fs = extract_files(resp.content)
    if len(fs) == 0:
        debug_path = os.path.join(os.path.dirname(__file__), "debug_vue_part2.txt")
        with open(debug_path, "w", encoding="utf-8") as f:
            f.write(resp.content[:3000])
        print(f"  [调试] 0个文件, 保存到 {debug_path}")
    print("  %d个: %s" % (len(fs), [f[0] for f in fs]))
...
# ====== 7. 创建项目文件 ======
def create_project(state: WorkflowState) -> dict:
    ...
    files = []
    for filepath, content in raw_files:
        filepath = filepath.strip()
        # Skip paths with newlines (malformed extraction)
        if "\n" in filepath or "\r" in filepath:
            print(f"  [跳过] 路径包含换行: {filepath[:80]}...")
            continue
        # Skip paths that are too long (likely code leaked into path)
        if len(filepath) > 200:
            print(f"  [跳过] 路径过长({len(filepath)}): {filepath[:80]}...")
            continue
        # Skip if content is empty
        if not content.strip():
            print(f"  [跳过] 空内容: {filepath}")
            continue
    ...
    for filepath, content in files:
        ...
        try:
            ...
            print(f"  [错误] 写入失败 {filepath}: {e}")

    print("[7/8] 创建项目: 后端%d个 + 前端%d个 = %d个文件" % (be_count, fe_count, len(files)))
...
# ====== 8. 自动构建与修复 ======
def auto_build_fix(state: WorkflowState) -> dict:
    ...
    if project_dir and os.path.isdir(project_dir):
        try:
            ...
            mvn_ok = type('', (), {'returncode': 1})()
        if mvn_ok.returncode != 0:
            print("  [跳过] Maven 未安装，跳过后端编译检查")
        else:
            print(f"  Maven 可用")
            be_built = _try_compile_backend(project_dir, max_attempts)
            if be_built:
                print("  [后端] 编译成功!")
            else:
...
def _try_build_frontend(frontend_dir: str, max_attempts: int) -> bool:
    for attempt in range(max_attempts):
        ...
        if r.returncode != 0:
            ...
            if attempt < max_attempts - 1:
                print("  → 调用AI修复...")
                _fix_and_rewrite(err, frontend_dir, "Vue前端")
                continue
            return False
...
def _try_compile_backend(project_dir: str, max_attempts: int) -> bool:
    for attempt in range(max_attempts):
        ...
        if r.returncode != 0:
            ...
            if attempt < max_attempts - 1:
                print("  → 调用AI修复...")
                _fix_and_rewrite(err, project_dir, "Spring Boot后端")
                continue
            return False
...
# ====== 构建工作流 ======
# --draft: skip auto-fix (for superpowers review later)
# --full (default): all 8 steps including auto-fix

def build_agent(draft_mode: bool = False):
    """Build and compile the workflow agent."""
    all_steps = [
        ("docs",                  generate_docs),
...
agent = build_agent(draft_mode=False)
...
def main():
    ...
    if draft_mode:
        draft_agent = build_agent(draft_mode=True)
        result = draft_agent.invoke({"requirement": req})
    else:
        result = agent.invoke({"requirement": req})
